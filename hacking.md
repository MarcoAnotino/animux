# Hacking animux
animux is derived from ani-cli and supports the providers maintained by this fork. Keep provider changes focused and test them independently.

Because animux is open source and streaming sites often share similar structures, you can adapt animux to a site that follows these conventions.
...
- writing regexes
You'll also need a web browser with a debugger and an environment that can run animux.
...
If you have done everything correctly, you can run `./animux`, query your site of choice and select from the responses.
Then animux should fail without a message.
If it fails with `No results found!` you have debugging to do.
Running `sh -x ./animux` is a good way to debug.
...
Then animux should fail with `Episode not released!`
...
## Other functionality
Assuming you completed all the necessary modifications, animux should completely work for you now.
...
There might be cases that cannot be covered by the current structure of animux, but it works for most sites with the expected layout.
